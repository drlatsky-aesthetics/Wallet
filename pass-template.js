...
export function buildPassTemplate({ serialNumber, targetUrl, memberName }) {
  return {
    ...
    // ── Pass fields (storeCard layout) ────────────────────────────────────
    // Layout order: primaryFields → secondaryFields → auxiliaryFields
    // backFields appear on the back of the pass when tapped
    storeCard: {
      headerFields: [
        {
          key:   "status",
