...
export async function buildStrip({ memberName, membershipTier = "standard" }) {
  ...
  // Left-justified brand-serif name (thin Cormorant cut) with the tier word
  // bigger, 60% opacity, slightly overlapping the name's tail.
  // Wallet's strip crop is CENTERED on wide iPhones (~6-7% lost per side,
  // confirmed on-device: left-set text lost its first letter, right-set text
  // its tail) — keep all text inside a 100px inset on BOTH sides @3x.
  const LEFT = 150;
  const RIGHT_SAFE = W - 100;
  let nameSize = 96;
  let tierSize, nameW, tierW, overlap;
  do {
    tierSize = Math.round(nameSize * 0.72);
    // Owner 2026-08-04: more offset between name and tier — a small
    // gap (negative overlap) instead of intrusion; layering stays via the
    // dropped baseline + 60% opacity.
    overlap = -Math.round(tierSize * 0.34);
  ...
  ${glyphPaths(BRAND.italic, tier.word, startX + nameW - overlap, baseY + tierSize * 0.34, tierSize, tier.color, 'opacity="0.6"')}
