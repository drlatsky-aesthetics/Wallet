...
export function buildPassTemplate({ serialNumber, targetUrl, memberName, referralCode, planUrl, membershipTier, refreshUrl }) {
  ...
  return {
    ...
    // Treasury Aesthetics app (resolves from https://link-to.app/CPCJEF0k5-6Qf8gqAtuNPQ).
    // Wallet renders a native app banner on the pass details — "Open" when
    // installed, "GET" to download — with the app's own icon. Custom tappable
    // logos on the pass front aren't allowed by Apple.
    associatedStoreIdentifiers: [6788675751],
    ...
    storeCard: {
      ...
      // No Wallet-rendered primary field: the strip artwork itself carries
      // the member name (smaller size) with the tier word in italics beside
      // it, tier-coloured — see lib/strip-image.js. memberName still feeds
      // that renderer via generate-pass-buffer.
      primaryFields: [],
      ...
      secondaryFields: [
        ...
        {
          // Small bottom-right pointer to Apple's flip control — the pass
          // back holds plan link, booking, contact, and Refresh My Pass.
          key:   "details",
          label: "DETAILS",
          value: "Tap •••",
          textAlignment: "PKTextAlignmentRight",
      ...
      backFields: [
        {
          ...
          value:
            ...
            "Clinical precision meets luxury care.",
        },
        {
          key:             "plan",
        ...
        {
          key:             "app",
          label:           "Treasury Aesthetics App",
          value:           "Book, browse, and manage your visits",
          attributedValue: "<a href='https://link-to.app/CPCJEF0k5-6Qf8gqAtuNPQ'>Open or download the app</a>",
        },
        {
          key:             "contact",
          label:           "Contact",
