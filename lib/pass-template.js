...
// membershipTier: "standard" | "vault" | "reserve"
// Controls the header badge and membership label on the pass.
export function buildPassTemplate({ serialNumber, targetUrl, memberName, membershipTier = "standard" }) {
  ...
  const tierLabel = tier === "vault"    ? "VAULT"
                  : tier === "reserve"  ? "RESERVE"
                  :                      "MEMBER";
  const tierValue = tier === "vault"    ? "Vault Member"
  ...
  return {
    ...
    // Logo text next to logo image — keep short, all-caps to match brand
    logoText: "TREASURY",
